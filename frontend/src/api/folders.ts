...
export const foldersApi = {
  ...
  getFolder: async (id: number): Promise<Folder> => {
    const response = await api.get<ApiResponse<Folder>>(`/folders/${id}`)
    return response.data.data
  },
  ...
  updateFolder: async (id: number, data: UpdateFolderRequest): Promise<Folder> => {
    const response = await api.patch<ApiResponse<Folder>>(`/folders/${id}`, data)
    return response.data.data
  },

  deleteFolder: async (id: number): Promise<void> => {
    await api.delete<ApiResponse<void>>(`/folders/${id}`)
  },
}
